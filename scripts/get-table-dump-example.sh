...
# dump command.
docker exec -it ${DATABASE_CONTAINER_NAME} mysqldump -u ${DATABASE_USER} -p${DATABASE_PASSWORD} ${DATABASE_NAME} ${TABLE_NAME} --master-data --single-transaction > ${OUTPUT_FILE}
